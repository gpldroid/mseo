...
for (const file of files) {
  ...
  const lastmod = info.mtime.toISOString().slice(0, 10);
  const path = file === "index.html" ? "" : "/" + file;
  urls.push(`  <url><loc>${baseUrl}${path}</loc><lastmod>${lastmod}</lastmod><changefreq>${changefreq(file)}</changefreq><priority>${priority(file)}</priority></url>`);
}
